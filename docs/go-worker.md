...
- Reporting: streams live log chunks to the control-plane and posts manifest/logs/plan/events when `CONTROL_PLANE_URL`/`CONTROL_PLANE_TOKEN` are set; sends heartbeats to `/api/worker/heartbeat`; always writes manifest locally.
- Endpoints: `/health`, `/ready`, `POST /trigger` (optional `WORKER_TOKEN`), `/plan` (GET existing, POST to regenerate).
- Config (env-driven): `QUEUE_BACKEND`, `QUEUE_FILE`, `REDIS_URL`, `REDIS_KEY`, `KAFKA_BROKERS`, `KAFKA_TOPIC`, `OUTPUT_DIR`, `CACHE_DIR`, `CACHE_MAX_BYTES`, `CACHE_PRUNE_INTERVAL_SEC`, `PYTHON_VERSION`, `PLATFORM_TAG`, `INDEX_URL`, `EXTRA_INDEX_URL`, `UPGRADE_STRATEGY`, `PLAN_OVERRIDES_JSON`, `MAX_DEPS`, `REQUIREMENTS_PATH`, `CONTAINER_IMAGE`, `CONTAINER_PRESET`, `WORKER_TOKEN`, `CONTROL_PLANE_URL`, `CONTROL_PLANE_TOKEN`, `WORKER_ID`, `WORKER_RUN_ID`, `WORKER_HEARTBEAT_INTERVAL_SEC`, `WORKER_AUTORUN_INTERVAL`, `PODMAN_BIN` (default `podman`), `WORKER_RUN_CMD`, `RUNNER_TIMEOUT_SEC`, `REQUEUE_ON_FAILURE`, `MAX_REQUEUE_ATTEMPTS`, `BATCH_SIZE`, `CAS_MAX_PARALLEL`, `OBJECT_STORE_MAX_PARALLEL`, `INFER_ENABLED`, `INFER_URL`, `INFER_TOKEN`, `INFER_MODEL`, `INFER_TIMEOUT_SEC`, `INFER_MAX_RETRIES`, `INFER_SYSTEM_PROMPT`, `INFER_USER_PROMPT_TEMPLATE`, `OBJECT_STORE_*`.
- Prompting: `INFER_URL` and `INFER_TOKEN` stay environment-only. In normal operation the worker refreshes `infer_enabled`, model, timeout/retry policy, and prompt templates from the control-plane `/api/settings` endpoint so prompt tuning happens at the app layer instead of in container secrets.
