...
- Build/run the API/worker container (Python toolchain, build tools) and UI container (static SPA served on 3000).
- Wire volumes `/input`, `/output`, `/cache`, and pass worker token/webhook/env via compose.
- Allow local development (`npm run dev`, `uvicorn`), and production builds via Dockerfiles.
...
**Current status**
- Dockerfiles for API/worker and UI exist; docker-compose includes api, worker, ui services.
