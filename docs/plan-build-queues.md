...
## Target Architecture
- **Uploader**: API/UI uploads immutable inputs to object storage and records them as pending inputs.
...
- **Settings**: add `auto_plan` and `auto_build` to `/api/settings`.
...
## Worker Changes
- **Plan worker mode** (new): pops `plan_queue`, reads referenced object from MinIO (or metadata), runs planner, stores plan (DB + CAS/object store or JSONB), updates plan status; if `auto_build` is true, enqueues plan_id to `build_queue`.
- **Build worker**: unchanged core logic; input is plan_id popped from `build_queue`.
- **Uploader**: inserts `pending_inputs`, and either enqueues to `plan_queue` (auto) or leaves for manual action.

## Data Flow (auto)
1) Upload API stores input object → insert `pending_inputs` → enqueue to `plan_queue`.
...
1) Upload API inserts `pending_inputs` only.
...
2) Settings: add `auto_plan`, `auto_build` to `/api/settings`.
3) Queues: add Redis keys `plan_queue`, `build_queue`; helpers.
4) APIs: pending-inputs and plans endpoints; enqueue actions.
5) Upload API: store object + insert/enqueue (configurable auto/manual).
