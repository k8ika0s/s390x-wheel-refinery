...
### Local dev (compose)
- `podman-compose.yml` now brings up Zot on `http://localhost:5000` (repo default: `artifacts`) and MinIO on `http://localhost:9000` (console `:9001`) with credentials `minio/minio123`.
